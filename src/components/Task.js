...
function Task({
  id, time, desc, completed, deleted, startEdit,
}) {
  const dispatch = useDispatch();
  const [contextMenuOpen, setContextMenuOpen] = useState(false);
  const inputText = useRef();
  function completeHandler() {
    inputText.current.classList.add('anim');
    setTimeout(() => { dispatch(toggleCompleteTodo(id)); }, 250);
  }
  ...
  function menuDeleteClick() {
    inputText.current.classList.add('anim');
    if (deleted) {
      setTimeout(() => { dispatch(deletePermanentlyTodo(id)); }, 250);
    } else {
      setTimeout(() => { dispatch(deleteTodo(id)); }, 250);
    }
  }
  ...
  function menuRecoverClick() {
    if (deleted) {
      inputText.current.classList.add('anim');
      setTimeout(() => { dispatch(recoverTodo(id)); }, 250);
    }
  }
  ...
  return (
    <li className="task-wrapper">
      <div className="task-row">
        ...
        <div className={`task-time ${completed && 'completed'} ${deleted && 'deleted'}`}>{time}</div>
        <div ref={inputText} className={`task-desc ${completed && 'completed'} ${deleted && 'deleted'}`} title="Left Click - mark as done." tabIndex={-1} role="button" onClick={completeHandler} onKeyDown={completeHandler}>{desc}</div>
      </div>
    </li>
  );
}
